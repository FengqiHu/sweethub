...
$ipkiki = "select * from black_ip order by id desc";
        ...
        <div class="card">
            <div class="card-body">
                <h4 class="header-title mb-3 size_18">IP封禁管理
                    <a class="fabu" href="/admin/addBlackIP.php">
                ...
                <table id="basic-datatable" class="table dt-responsive nowrap" width="100%">
                    ...
                    while ($IPinfo = mysqli_fetch_array($ipki)) {
                        ...
                        <tr>
                            ...
                            <td>
                                <small class="text-muted"><?php echo $IPinfo['updated_time'] ?></small>
                            </td>
                            <td>
                                <h5><span class="badge badge-success-lighten"> <?php echo $IPinfo['note'] ?></span></h5>
                            </td>
                            <td>
                                <h5>
                                    <span class="badge badge-danger-lighten"><?php if ($IPinfo['ip']) { ?><?php echo $IPinfo['city'] ?><?php } else { ?>127.0.0.1<?php } ?></span>
                                </h5>
                            </td>
                            <td>
                                <a href="javascript:del(<?php echo $IPinfo['id']; ?>,'<?php echo $IPinfo['ip']; ?>');">
...
<script>
    function del(id, ip) {
        if (confirm('您确认要删除IP为 ' + ip + ' 吗')) {
            $.ajax({
                url: 'posts/deleteBlackIPPost.php',
                type: 'POST',
                data: {
                    id: id
                },
                ...
                success: function(response) {
                    if (response.status == 'success') {
                        toastr["success"](response.message, "SweetHub");
                        // 删除成功后，从表格中移除该行
                        setTimeout(function() {
                            location.reload(); // 重新加载页面刷新数据
                        }, 1000);
                    } else {
                        toastr["error"](response.message, "SweetHub");
                    }
                },
                error: function() {
